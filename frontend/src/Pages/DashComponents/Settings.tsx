...
const SensorManagement: React.FC = () => {
  ...
  const fetchSensors = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:5000/api/settings/sensors"
      );
      setSensors(response.data);
      setTotalActive(
        response.data.filter((sensor: Sensor) => sensor.status).length
      );
  ...
  const handleSave = async () => {
    try {
      if (editingSensor) {
        ...
        await axios.put(
          `http://localhost:5000/api/settings/sensors/${editingSensor.id}`,
          {
            id: editingSensor.id,
            ...formValues,
          }
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/settings/sensors/${deletingSensorId}`
      );
      setSnackbar({
        open: true,
  ...
  const toggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      await axios.put(
        `http://localhost:5000/api/settings/sensors/${id}/status`,
          ...
          >
            <Grid item>
              <Box>
                ...
                <Typography
                  variant="body1"
                  sx={{ color: colors.textSecondary, opacity: 0.7 }}
                >
            ...
            <Grid item>
              <Paper
                elevation={1}
                sx={{ borderRadius: "16px", padding: "8px 16px" }}
              >
                      ...
                      <TableCell>
                        ...
                        >
                          ...
                          <Switch
                            ...
                            onChange={() =>
                              toggleStatus(sensor.id, sensor.status)
                            }
                            color="success"
                          />
                        </Box>
            ...
            <Grid container spacing={2} sx={{ mb: 2 }}>
              <Grid item xs={6}>
                <TextField
                  ...
                  InputProps={{
                    startAdornment: (
                      <ExploreIcon color="action" sx={{ mr: 1 }} />
                    ),
                  }}
